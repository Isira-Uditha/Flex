        ...
        <header style=" border: 1px solid white">
            <table style="width:100%; margin-top: 0%;">
                <tr style="width:100%" >
                    ...
                    <td style="width:70%"  align="right"  valign="bottom"><h1><span style="font-style:italic; color:#b6b4b4; font-size:16px">Appointment Report : {{date('Y-m-d')}}</span></h1></td>
                </tr>
            </table>
            <hr />
        ...
        <div  class="center content" style="margin-top:3%;  border: 1px solid white">
            <table width="100%" id="appointment_table">
                ...
                <tbody>
                    @foreach ($data as $res)
                        <tr>
                            <td scope="col">{{$res->appointment_id}}</td>
                            <td scope="col" >{{$res->appointment_no}}</td>
                            <td scope="col">{{$res->appointment_date}}</td>
                            <td scope="col">{{$res->time_slot}}</td>
                            <td scope="col">{{$res->workout_plan_id}}</td>
                            <td scope="col">{{$res->bmi}}</td>
                            @if($res->bmi_type == 'Normal weight')
                                <td scope="col" style="text-align: right; color: green">{{$res->bmi_type}}</td>
                            @elseif ($res->bmi_type == 'Obesity')
                                <td scope="col" style="text-align: right; color: red">{{$res->bmi_type}}</td>
                            @elseif ($res->bmi_type == 'Underweight')
                                <td scope="col" style="text-align: right; color: red">{{$res->bmi_type}}</td>
                            @elseif ($res->bmi_type == 'Overweight')
                                <td scope="col" style="text-align: right; color: rgb(175, 175, 13)">{{$res->bmi_type}}</td>
                            @endif
                        </tr>
                    @endforeach
                </tbody>
